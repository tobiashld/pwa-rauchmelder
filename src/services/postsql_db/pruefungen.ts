...
async function get(
	...
	cb?: (data: DBResponse<Pruefung>) => void
): Promise<DBResponse<Pruefung>> {
	const url =
		dynamicurl +
		"/pruefungen" +
		(params
		...
		.then((obj: DBResponse<Pruefung>) => {
			...
			return {
				...
				data: obj.data!.map((item: any) => {
					return toPruefungConverter(item);
				}),
			};
		});
}

async function add(pruefung: any, cb?: (data: any) => void) {
...
async function change(pruefung: any, cb?: (data: any) => void) {
