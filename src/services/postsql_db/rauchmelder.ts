...
): Promise<{ status: number; data: RauchmelderBeziehung[]; error?: string }> {
	const url =
		dynamicurl +
		"/rauchmelder" +
		(params
	...
	return fetch(url, {
		...
		.then((obj) => {
			if (cb)
				cb({
					...obj,
					data: obj.data.map((rauchmelderBz: any) =>
						toRauchmelderBzConverter(rauchmelderBz)
					),
				});
...
): Promise<DBResponse<RauchmelderHistorie>> {
		...
		.then((obj) => {
			...
			return {
				...
				data: obj.data?.map((item: any) =>
					toRauchmelderHistorienConverter(item)
				),
			};
		});
}
...
async function switchRauchmelder(
	neuerRauchmelder: Rauchmelder,
	alteRauchmelderBz: RauchmelderBeziehung
) {
	...
	fetch(url, {
		...
		body: JSON.stringify({
			newRauchmelder: neuerRauchmelder,
			altRauchmelderBz: alteRauchmelderBz,
		}),
	})
		.then((response) => {
			return response.json();
