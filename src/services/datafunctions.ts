...
const dataFunctions = {
	[ClientStatus.offline]: {
		pruefungen: {
			...
			deletePruefung: (pruefung: Pruefung) =>
				db.table("pruefungen").delete(pruefung.id),
		},
		rauchmelder: {
			get: (
				props: { [key: string]: number } | undefined,
				cb?: (data: DBResponse<RauchmelderBeziehung>) => void
			) => {
				if (props) {
					db.table("rauchmelder")
						.filter(
							(rauchmelder) =>
								rauchmelder[Object.keys(props)[0]] ===
			...
			addRauchmelder: (geprRauchmelder: GeprRauchmelder) =>
				db
					.table("gepruefteRauchmelder")
					.add(geprRauchmelder, geprRauchmelder.id),
			deleteRauchmelder: (geprRauchmelder: GeprRauchmelder) =>
				db.table("gepruefteRauchmelder").delete(geprRauchmelder.id ?? -1),
			getAllRauchmelder: () => db.table("gepruefteRauchmelder").toArray(),
			getRauchmelderWithParams: () =>
				db.table("gepruefteRauchmelder").toArray(),
		},
		auftraggeber: {
			get: (props: any, cb?: (data: any[]) => void) => {
				db.table("auftraggeber")
					.toArray()
					.then((data) => {
						if (cb) {
							cb(data);
