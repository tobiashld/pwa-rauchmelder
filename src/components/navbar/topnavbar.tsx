...
function TopNavBar(props: { isShown: boolean; onMenuChange: () => void }) {
						...
						if (chat.nachrichten) {
							...
							chat.nachrichten = [
								...
								new Message(
									...
									chat?.id,
									userUUID,
									{ username: username ? username : "ERROR", email: "test" },
									true
								),
							];
	...
	return (
		...
		>
			{width < 800 ? (
				<div
					className={styles.bmcontainer}
					onClick={props && props.onMenuChange ? props.onMenuChange : undefined}
				>
					<div className={styles.menuIcon}>
						<svg
							className={
								"ham hamRotate ham1 " + (props && props.isShown ? "active" : "")
							}
							viewBox="0 0 100 100"
							width="60"
							height="60"
			...
			>
				<div style={{ position: "relative" }}>
					...
					<Tooltip title={"Nachrichten"}>
						...
						>
							<Badge
								badgeContent={messageCount ? messageCount : undefined}
								color="error"
							>
								<Notifications />
							</Badge>
				...
				>
					<MenuItem
						className={styles.menueitem}
						onClick={(event) => handleClose(event, "profile")}
					>
						<ListItemIcon>
							<Avatar
								{...stringAvatar(
									username && username.length > 2 && username !== "0"
										? username
										: "Test mann"
								)}
							>
					...
					<MenuItem
						className={styles.menueitem}
						onClick={(event) => handleClose(event, "settings")}
					>
						<ListItemIcon>
					...
					<MenuItem
						...
						onClick={() => {
							enqueueSnackbar("Erfolgreich ausgeloggt!", {
								variant: "success",
							});
							dispatch(logout());
						}}
