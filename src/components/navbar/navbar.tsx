...
function NavBar(props: {
	isShown: boolean;
	changeComponent: (route: string) => void;
}) {
	...
	return (
		<div
			className={
				styles.widthcontroller + (props.isShown ? ` ${styles.fullwidth}` : "")
			}
		>
			<Box
				sx={{ backgroundColor: "secondary" }}
				className={
					styles.container + (props.isShown ? ` ${styles.navactive}` : "")
				}
			>
					...
					{navbarElemente.map((item, index) => {
						return (
							<NavBarLink
								...
								name={item.name}
								icon={item.icon}
								onClick={() => props.changeComponent(item.route)}
							/>
						);
					})}
				</div>
