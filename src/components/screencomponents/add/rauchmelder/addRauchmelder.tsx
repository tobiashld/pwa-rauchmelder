...
function AddRauchmelder() {
	const [rauchmelder, setrauchmelder] = useState<Rauchmelder[]>([
		new Rauchmelder("", "", new Date(), true, undefined),
	]);
	const [isSavable, setIsSavable] = useState(false);
	const queryClient = useQueryClient();
	const navigate = useNavigate();
	const objekteQuery = useObjekteGet();
	const wohnungsQuery = useQuery("wohnungen", () =>
	...
	const handleChange = (key: keyof Rauchmelder, value: any, id: number) => {
		setIsSavable(true);
		setrauchmelder(
			rauchmelder.map((rauchmelder: Rauchmelder, index) => {
	...
	if (
		wohnungsQuery.isLoading ||
		objekteQuery.isLoading ||
		!wohnungsQuery.data?.data ||
		!objekteQuery.data?.data
	) {
					...
					{rauchmelder.map((rauchmelder, index) => {
						return (
							<div className={styles.datarowelement} onClick={(e) => {}}>
								<div className={styles.rowsegment}>
									<TextInput
										size={"Small"}
										placeholder={"Raum"}
										onChange={(event) => {
											handleChange("raum", event.currentTarget.value, index);
										}}
									/>
								</div>
								<div className={styles.rowsegment}>
								...
								<div className={styles.rowsegment}>
									<TextInput
										...
										placeholder={"Objekt"}
										onChange={(event) => {
											handleChangeObjekt(event.currentTarget.value, index);
										}}
									/>
								</div>
								<div className={styles.rowsegment}>
									<TextInput
										...
										placeholder={"Wohnung"}
										onChange={(event) => {
											handleChangeWohnung(event.currentTarget.value, index);
										}}
									/>
								</div>
							</div>
